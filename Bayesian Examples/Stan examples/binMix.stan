...
model {
  ...
  for (i in 1:R) {
    ...
    for (j in 1:(K - max_y[i] + 1)) //Loops across both lpmf functions going from max_y[i]+1 to K. Very counter-intuitive...
      lp[j] = poisson_lpmf(max_y[i] + j - 1 | lambda)
             + binomial_lpmf(y[i] | max_y[i] + j - 1, p);
    target += log_sum_exp(lp); //Integrated likelihood across max_y[i] to K -- "target +=" increments likelihood
  } //Conclusion: 
}